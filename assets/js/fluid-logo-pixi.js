class FluidLogoPixi {
    ...
    createParticlesFromImage(img) {
        ...
        // High quality - step of 1.2 for great clarity
        const step = 1.2;
        let particleCount = 0;
        ...
        for (let py = 0; py < sh; py += step) {
            for (let px = 0; px < sw; px += step) {
                ...
                if (alpha > 25) {  // Lower threshold for better coverage
                    ...
                    particle.drawCircle(0, 0, 3.5);  // Larger for better quality
                    particle.endFill();
    ...
    setupEventListeners() {
        ...
        this.canvasElement.addEventListener('mouseleave', handleMouseLeave);
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (logoCanvas) {
        ...
        if (logoLink && logoContainer) {
            ...
            // Handle clicks ONLY within logo bounds (where particles actually are)
            ...
            // Wait for PixiJS to initialize before adding click handler
            setTimeout(() => {
